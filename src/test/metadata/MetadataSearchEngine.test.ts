...
describe("MetadataSearchEngine", () => {
	...
	describe("search with no filters", () => {
		...
		it("should handle offset and limit together", async () => {
			...
			expect(result.total).toBe(4);
			expect(result.hasMore).toBe(true);
			expect(result.results[0].relativePath).toBe("tech-context.md");
		});
	...
	describe("text search", () => {
		it("should search in title", async () => {
			...
			expect(result.results[0].title).toBe("AI Memory Extension");
		});
		...
		it("should search in relative path", async () => {
			...
			expect(result.results).toHaveLength(1);
			expect(result.results[0].relativePath).toBe("notes/research.md");
		});
		...
		it("should handle partial matches", async () => {
			...
			expect(result.results[0].relativePath).toBe("tech-context.md");
			expect(result.results[0].type).toBe("techContext");
		});
	});
	...
	describe("type filtering", () => {
		it("should filter by type", async () => {
			...
			expect(result.results).toHaveLength(1);
			expect(result.results[0].type).toBe("projectBrief");
		});
		...
		it("should handle undefined type filtering", async () => {
			const result = await searchEngine.search({ type: undefined });
	...
	describe("tag filtering", () => {
		it("should filter by single tag", async () => {
			...
			expect(result.results).toHaveLength(1);
			expect(result.results[0].tags).toContain("extension");
		});
		...
		it("should handle entries without tags", async () => {
			...
			expect(result.results).toHaveLength(1);
			expect(result.results[0].relativePath).toBe("project-brief.md");
		});
	});
	...
	describe("validation status filtering", () => {
		...
		it("should filter by validation status invalid", async () => {
			...
			expect(result.results).toHaveLength(1);
			expect(result.results[0].validationStatus).toBe("invalid");
		});
		...
		it("should filter by validation status unchecked", async () => {
			...
			expect(result.results).toHaveLength(1);
			expect(result.results[0].validationStatus).toBe("unchecked");
		});
	});
