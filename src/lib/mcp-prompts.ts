...
export const MEMORY_BANK_HEALTH_CHECK_PROMPT = "Please check the health of the Memory Bank and report any missing or corrupted files. Suggest repairs if needed.";
...
export const MEMORY_BANK_STRUCTURE_GUIDE_PROMPT = "Refer to the Memory Bank structure diagram in the ruleset to understand the relationship between files and modules. Use this as a guide when reading or updating files.";

export const MEMORY_BANK_USAGE_TIP_PROMPT = "Tip: Regularly review the Memory Bank for outdated or incomplete information. Keeping it up to date improves agent performance and project continuity.";

export const REVIEW_AND_UPDATE_MEMORY_BANK_PROMPT = "You are reviewing all memory bank files. For each file, review the content and suggest updates if needed. Always prompt before overwriting protected files. After all files are reviewed, summarise the changes made.";
