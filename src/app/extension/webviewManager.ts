...
export class WebviewManager {
	...
	private readonly memoryBankService: MemoryBankManager;
	...
	constructor(
		...
		memoryBankService: MemoryBankManager,
	...
	private async handleRequestMemoryBankStatus() {
		...
		const result = await this.memoryBankService.getIsMemoryBankInitialized();
		const isInitialized = result.success ? result.data : false;
	...
	private async performRulesReset(): Promise<{
		...
		// Access FileOperationManager directly from the memory bank service
...
<body>
  ...
  <script type="module" src="${scriptUri}" nonce="${nonce}"></script>
	...
	private async sendCurrentMCPServerStatus(): Promise<void> {
		...
		try {
			// Check if server is running using the interface method
			const isRunning = this.mcpServer.isServerRunning?.() ?? false;
