/**
 * @file src/core/file-operations.ts
 * @description Provides a robust manager for secure file system operations.
 *
 * This class handles all file system interactions with built-in retry logic,
 * timeouts, and critical security validations to prevent directory traversal attacks.
 */
...
import type { FileError, FileOperationManagerConfig, RetryConfig } from "../lib/types/operations";
import { validateMemoryBankPath } from "../lib/validation";
...
export class FileOperationManager {
	 ...
	 * SECURITY: Path is validated before any file access
	 */
	async mkdirWithRetry(path: string, options?: { recursive?: boolean }): Promise<Result<void, FileError>> {
		try {
			const validatedPath = this.validatePath(path);
			return await this.withRetry(async () => {
	...
	private createFileError(error: unknown, operation: string): FileError {
		if (error instanceof Error) {
			const nodeError = error as NodeJS.ErrnoException;
			return {
				code: nodeError.code ?? "UNKNOWN_FILE_OPERATION_ERROR",
		...
		return {
			code: "UNKNOWN_ERROR",
			message: `${operation} failed with non-error object: ${String(error)}`,
