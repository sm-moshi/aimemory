	...
	try {
		...
		if (fileOperationManager) {
			try {
				...
				logger.warn("Failed to load template, using fallback content", { error });
 ...
 */
export function getInitialConfiguration(): { logLevel: LogLevel; config: vscode.WorkspaceConfiguration } {
	const config = getConfiguration();
 ...
 */
export function createWorkspaceManager(context: vscode.ExtensionContext, container: DIContainer) {
	// Validate workspace on creation
	validateWorkspaceForExtension();
